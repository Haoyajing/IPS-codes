...
if len(runNets) == 1:
    netName = runNets[0]
    filename = f'ncSize_SIR_{netName}_robust_range_mu{muName}.json'
elif len(runNets)==20:
     filename = f'ncSize_SIR_all_robust_range_mu{muName}.json'
else:
    filename = f'ncSize_SIR_multi_robust_range_mu{muName}.json'
if mu == 1:
    nets_paras = {
...
def main():
    ...
    # results = [([k1,v1],[ke1,va1]), ([k2,v2],[ke2,va2]), ...] 有len(args)个元素
    end_time = time.time()
