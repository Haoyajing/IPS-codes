...
if len(runNets) == 1:
    netName = runNets[0]
    filename = f'SIR_{netName}_robust_range_mu{muName}.json'
elif len(runNets)==20:
     filename = f'SIR_all_robust_range_mu{muName}.json'
else:
    filename = f'SIR_multi_robust_range_mu{muName}.json'
if mu == 1:
    nets_paras = {
...
def main():
    ...
    for net in runNets:
        for nu in [1,2,3,4]:
            ...
            for l in [lThres*0.1, lThres, lThres*10]:
