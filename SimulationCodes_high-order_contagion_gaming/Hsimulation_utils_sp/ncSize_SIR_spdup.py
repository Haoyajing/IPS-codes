...
class Simu_NCsize_SIR():
    ...
    def initialization(self, initial_infected, immunilized=[]):  # SIR requires explicitly specifying initial infected nodes
        ...
        for to_infect in self.infect_this_setup:  # to_infect is a node index
    ...
    def contagionSimulation(self, l, mu, nu, tmax=float("Inf"), timeSeries=False):
        ...
        while t < tmax and I[-1] != 0:
            ...
            newI = []
            """
            potentialI = set()  # potential susceptible nodes to speed up the simulation
            ...
            for node in self.sAgentSet:
                p = 1
                for edgeID in self.HneiEdgeDic[node]:  # list of incident hyperedges for this node
                    p = p * infectionProbLst[iNodesNum_inHedge[edgeID]]
                if np.random.random() < 1 - p:
                    newI.append(node)
            
            # recovery step; cannot modify the set while iterating, so use a temporary set newR
            newR = set()
    ...
    def prepare_for_numba(self):
        # 1. convert nodeStateDic to a np.array with shape [max_node_index + 1, 3]
        # column 0 = infected, column 1 = recovered, column 2 = susceptible
        ...
        # this is intentionally not synchronized with initialization because it will be recomputed
        # at the beginning of each while-loop iteration during simulation
...
def dict2numba(dic):
    """Convert a Python dict into a numba-typed dict (int64 keys, 1D int64 arrays as values)."""
...
def dict2numba_float(dic):
    """Convert a Python dict into a numba-typed dict (int64 keys, 1D float64 arrays as values)."""
    output = nb.typed.Dict.empty(
        key_type = nb.types.int64, 
        value_type = nb.types.Array(nb.types.float64, 1, 'C')
    )
    for key, value in dic.items():
        output[key] = np.array(value, dtype=np.float64)
    return output

@njit
def contagion_simulation_numba(nodeStates, iNodesNum_inHedge, HneiEdgeDic, HedgeDic, HedgeSizeDic, infectionProbDic_numba, mu, tmax):
    ...
    while t < tmax and I[-1] != 0:
        ...
        for edgeID in HedgeDic:
            # use HedgeDic[edgeID] as indices to sum over nodeStates and obtain the infected count
            iNodesNum_inHedge[edgeID] = np.sum(nodeStates[HedgeDic[edgeID], 0])

        newI = []
        # directly use nonzero on the third column to get indices of susceptible nodes
        indexes = np.nonzero(nodeStates[:, 2])[0]
        for i in indexes:
            p = 1
            for edgeID in HneiEdgeDic[i]:
                p *= infectionProbDic_numba[HedgeSizeDic[edgeID]][iNodesNum_inHedge[edgeID]]
            if np.random.random() < 1 - p:
                newI.append(i)
...
@njit
def contagion_simulation_numba_timeDetail(nodeStates, iNodesNum_inHedge, HneiEdgeDic, HedgeDic, HedgeSizeDic, infectionProbDic_numba, mu, tmax):
    # track the counts of nodes in different states
    I = [np.sum(nodeStates[:, 0])]
    R = [np.sum(nodeStates[:, 1])]
    S = [np.sum(nodeStates[:, 2])]
